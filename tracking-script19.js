(function () {
  ...
  function getConfigFromQuery() {
    ...
    const trackingScript = Array.from(scripts).find(s => s.src && s.src.includes('tracking-script19'));
    ...
    const ctaTexts = (params.get('ctaText') || "").split(',').map(s => normalize(s));

    console.log('[Tracking] Configured CTA Texts:', ctaTexts);
  ...
  function handleClick(event) {
    ...
    console.log('[Click] Configured CTA Texts:', CONFIG.ctaTexts);
    ...
    if (CONFIG.ctaMethod === 'CTA Text') {
      ...
      console.log(`[Click] Text match: ${matchedCTA}`);
      if (matchedCTA) {
    ...
    if (CONFIG.ctaMethod === 'CTA Button Id') {
      ...
      console.log('[Click] Expected CTA ID:', CONFIG.ctaValue);

      const matchedId = ancestorId && ancestorId === CONFIG.ctaValue;
      console.log(`[Click] Button ID match: ${matchedId}`);
